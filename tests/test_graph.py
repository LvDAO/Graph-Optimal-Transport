...
def test_reversible_pi_kernel_wrapper_matches_eager_on_directed_example() -> None:
    ...
    wrapped_pi, wrapped_residual = _infer_pi_from_reversible_rates_kernel_jit(
        src,
        dst,
        rev,
        q,
    ...
    np.testing.assert_allclose(np.asarray(wrapped_pi), np.array([0.25, 0.5, 0.25]), atol=1e-12)
    assert float(wrapped_residual) < 1e-12


def test_reversible_pi_kernel_wrapper_matches_eager_on_larger_graph() -> None:
...
def test_reversible_pi_kernel_wrapper_is_alias_of_eager_kernel() -> None:
    src = np.array([0, 1, 1, 2], dtype=np.int32)
    dst = np.array([1, 0, 2, 1], dtype=np.int32)
    q = np.array([2.0, 1.0, 1.0, 2.0], dtype=np.float64)
    rev = np.array([1, 0, 3, 2], dtype=np.int32)
...
def test_reversible_pi_kernel_handles_strongly_varying_rates() -> None:
    ...
    assert np.all(np.isfinite(np.asarray(wrapped_pi)))
    assert np.all(np.asarray(wrapped_pi) > 0.0)
    np.testing.assert_allclose(np.asarray(eager_pi), ref_pi, atol=1e-12, rtol=1e-12)
    np.testing.assert_allclose(np.asarray(wrapped_pi), ref_pi, atol=1e-12, rtol=1e-12)
    np.testing.assert_allclose(np.asarray(wrapped_pi), pi_true, atol=1e-12, rtol=1e-12)
    np.testing.assert_allclose(float(eager_residual), ref_residual, atol=1e-12, rtol=0.0)
    np.testing.assert_allclose(float(wrapped_residual), ref_residual, atol=1e-12, rtol=0.0)
